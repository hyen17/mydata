(async () => {
  ...
  const autoSave = () => { // Made synchronous as image URL fixing is synchronous
    ...
    for (let i = 0; i < elementsToIterate.length; i++) {
      ...
      // Replace <img> tags with placeholder <div>s carrying data attributes
      ...
      imgTags.forEach(imgNode => {
          ...
          // Placeholder div will be empty, its data attributes will be used by renderHTML
          placeholderDiv.innerHTML = ``; 
          
          if (imgNode.parentNode) {
            imgNode.parentNode.replaceChild(placeholderDiv, imgNode);
            console.log(`[autoSave] Replaced <img> with placeholder div for src: ${newSrc}`);
          } else {
      ...
      const questionHtml = tempDivQ.innerHTML.trim();
      ...
      if (choiceUl) {
          ...
          while (nextSibling) {
              if (nextSibling.tagName === 'P') postChoiceHtml += nextSibling.outerHTML;
  ...
  const renderPDFs = () => { 
    ...
    const estimateHtmlHeight = (doc, html, fs = 12, currentUsableWidth = usableWidth, currentFontStyle = 'normal') => {
        ...
        tempDiv.querySelectorAll('div[data-is-image-placeholder="true"]').forEach(phNode => {
            textHeight += lineHeight; // For the placeholder text line itself
            ...
            if (htmlHeightStr && !isNaN(parseFloat(htmlHeightStr))) {
                ...
                imageReservedHeight += lineHeight * 5; // Default reserved space (approx 5 lines)
            }
        });
    ...
    const renderInlineText = (doc, textSegments, x, y, maxWidth, currentFontSize) => { 
        ...
        for (const segment of textSegments) {
            if (!segment.text.trim() && segment.text !== " ") continue; 
            doc.setFontSize(currentFontSize).setFont(undefined, segment.style);
            const normalizedText = segment.text.replace(/\s+/g, ' '); 
    ...
    const renderHTML = (doc, html, startY, initialStyle = 'normal', currentFontSize = 12, currentXIndent = margin) => {
      ...
      const processBlockNode = (node, yPos, style, listLevel = 0, listCounter = 1, listType = null, xIndent) => { 
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          // Handle placeholder divs generated by autoSave
          if (node.hasAttribute('data-is-image-placeholder')) {
            if (newY !== startY && newY > margin && newY < maxY - lineHeight) newY += lineHeight * 0.5;
            
            const imgSrc = node.getAttribute('data-img-src');
            const altText = node.getAttribute('data-img-alt') || 'Image';
            ...
            if (htmlWidthStr || htmlHeightStr) { // Check if either width or height is present
                placeholderText += ` | Dimensi HTML: ${htmlWidthStr || 'auto'}x${htmlHeightStr || 'auto'}`;
            }
            placeholderText += "]";
            
            console.log(`[renderHTML] Rendering placeholder div: ${placeholderText}`);
            ...
            if (htmlHeightStr && !isNaN(parseFloat(htmlHeightStr))) {
                ...
                console.log(`[renderHTML] Menyediakan ruang untuk gambar: ${pdfReservedHeightMm.toFixed(2)}mm (dari atribut HTML height ${htmlHeightPx}px)`);
                if (newY + pdfReservedHeightMm > maxY && newY > margin) { 
                    doc.addPage(); newY = margin;
                    // Re-render placeholder text on new page if it was split
                    doc.setFontSize(currentFontSize - 2).setFont(undefined, 'italic');
                    const linesOnNewPage = doc.splitTextToSize(placeholderText, usableWidth - (effectiveXIndent - margin));
                     for (const line of linesOnNewPage) { // This might re-render placeholder if it spanned pages, adjust if needed
                         if (newY + lineHeight > maxY + 0.5 && newY > margin + lineHeight/2) { doc.addPage(); newY = margin; }
                          doc.text(line, effectiveXIndent, newY);
                          newY += lineHeight;
                     }
                ...
                newY += lineHeight * 5; // Default reserved space (approx 5 lines) if no height attribute
                console.log(`[renderHTML] Tidak ada atribut height valid, menyediakan ruang default ${lineHeight * 5}mm.`);
            }
          ...
          if (tagName === 'BR') {
            ...
            for (const row of rows) {
                ...
                for (const cell of cells) {
                    ...
                    const cellTextContent = cell.innerText.replace(/\s+/g, ' ').trim();
  ...
  if (!document.getElementById('pdfToolsUI')) {
    ...
    // alert('Panel UI PDF Tool seharusnya muncul sekarang.'); // Alert might be annoying if UI appears
  } else {
