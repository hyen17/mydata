(async () => {
  ...
  const renderPDFs = () => { 
    ...
    // MODIFIED processInlineText to handle <br>
    const processInlineText = (parentNode, initialStyle = 'normal') => {
        ...
        const processChildren = (node, currentStyle) => {
            if (node.nodeType === Node.TEXT_NODE) {
                ...
                // Don't add if it's only whitespace after cleaning, unless it's a single space meant for flow
                if (text.length > 0 && (text.trim() !== '' || text === ' ')) {
                     segments.push({ text: text, style: currentStyle, type: 'TEXT' }); 
                }
                ...
                if (tagName === 'BR') {
                    segments.push({ type: 'BR' }); // Add BR segment
    ...
    // MODIFIED renderInlineText to handle BR segments and ensure page breaks
    const renderInlineText = (doc, textSegments, x, y, maxWidth, currentFontSize) => { 
        let currentX = x;
        let currentY = y;

        const ensureSpaceForOneLine = () => {
            if (currentY > maxY - lineHeight) { // If baseline is too low for one full line
                doc.addPage();
                currentY = margin;
                currentX = x; // Reset X for the new page
            }
        };
        ...
        for (const segment of textSegments) {
            if (segment.type === 'BR') {
                currentY += lineHeight;
                currentX = x; 
                ensureSpaceForOneLine(); // Check for page break after BR
                continue; 
            }

            let segmentText = segment.text;
            if (!segmentText || (!segmentText.trim() && segmentText !== " ")) continue; 

            doc.setFontSize(currentFontSize).setFont(undefined, segment.style);
            // Normalize multiple spaces to single, but keep leading/trailing for word splitting logic
            ...
            for (const word of words) {
                ...
                ensureSpaceForOneLine(); // Check page break BEFORE rendering word
      ...
      const processBlockNode = (node, yPos, style, listLevel = 0, listCounter = 1, listType = null, xIndent) => { 
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          if (node.hasAttribute('data-is-image-placeholder')) {
            ...
            // After renderInlineText, newY is the baseline of the last line of text.
            // Add lineHeight to move to the start of the *next* line for subsequent blocks.
            if (segments.some(s => (s.text && s.text.trim() !== '') || s.type === 'BR')) {
                 // If renderInlineText ended exactly at a line that could cause a page break for this added lineHeight
                if (newY > maxY - lineHeight && newY <= maxY) { // if it's on the last possible line
                    // don't add lineHeight if it pushes it off, let next block handle page break
                } else if (newY <= maxY - lineHeight) { // if there's space for this lineheight
                     newY += lineHeight;
                } else { // it's already past maxY or too close, page break will happen for next block
                    // no need to add lineHeight here, as newY is already effectively on next page's start or beyond
                }
            }
          ...
          if (tagName === 'BR') { 
            // This should be handled by processInlineText if BR is inside P/DIV.
            // If BR is a direct child of a block not handled by processInlineText (e.g. direct child of body):
            newY += lineHeight;
            if (newY + lineHeight > maxY + 0.5 && newY > margin + lineHeight/2) { doc.addPage(); newY = margin; }
