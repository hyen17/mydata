(async () => {
  ...
  const renderPDFs = () => { 
    ...
    const processInlineText = (parentNode, initialStyle = 'normal') => {
        ...
        const processChildren = (node, currentStyle) => {
            if (node.nodeType === Node.TEXT_NODE) {
                ...
                if (text.length > 0) segments.push({ text: text, style: currentStyle, type: 'TEXT' }); 
    ...
    // MODIFIED renderInlineText to handle BR segments
    const renderInlineText = (doc, textSegments, x, y, maxWidth, currentFontSize) => { 
        ...
        for (const segment of textSegments) {
            if (segment.type === 'BR') {
                currentY += lineHeight;
                currentX = x; // Reset X for new line
                if (currentY + lineHeight > maxY + 0.5) { // Check for page break after BR
      ...
      const processBlockNode = (node, yPos, style, listLevel = 0, listCounter = 1, listType = null, xIndent) => { 
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          if (node.hasAttribute('data-is-image-placeholder')) {
            ...
            // renderInlineText now returns Y at the baseline of the last text segment.
            // Add lineHeight only if content was actually rendered and to ensure next block starts on a new line.
            if (segments.some(s => s.text && s.text.trim() !== '')) {
                newY += lineHeight; 
            }
          ...
          if (tagName === 'BR') { // BR is handled by processInlineText now
            // This case in processBlockNode might be redundant if BR is only inside P/DIV
            // However, if <br> can be a direct child of other blocks, keep a simple handler
            newY += lineHeight;
            if (newY + lineHeight > maxY + 0.5 && newY > margin + lineHeight/2) { doc.addPage(); newY = margin; }
